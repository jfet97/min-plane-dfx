# Irregular Production Evolution

This page records production-level transitions. Search experiments that did not
ship are summarized separately in
[`search-quality-decisions.md`](./search-quality-decisions.md).

| Era | Key commits | Production consequence |
| --- | --- | --- |
| Convex baseline | early irregular-v2 integration through `ac75222` | real flattened/offset collision polygons, finite transforms, direct NFP/IFP legality, windowed beam, optional GA, real history |
| Exactness hardening | `95de72c`, `13396c5`, `f12b466`, `6d1cfa1` | canonical score/identity stability, exact endpoint authority, near-parallel crossing recovery, valid-circle quantization closure |
| Protected ordinary search | boundary/intrinsic/Pareto commits through `274265e` | bounded repair-disabled alternatives improved some historical layouts but did not close invariance |
| Fixed-reference handoff | `48054f7` through `ac32e94` | historical two-role compact path produced the `430,344.917527 mm2` ten-sheet Mixed result and a `120 s` timeout floor |
| Shared archive research | `ee9d0fa`, `e4378e5`, `4831035`, `fa9ab29`, `ed90f87` | deterministic periodic budgets, phase measurement, common exact archive, Triangle witness restoration, filtered source allocation |
| Third baseline | `ec6fb90`, `6d1cfa1`, `8f84f81`, `75b9699` | added the all-distinct Shapes-17 direct-archive gate |
| Archive-only production | `8b0fba4`, merge `f33831f`, recorded by `b506344` | removed the fixed coordinator/flag and ordinary compact fallback; Compact quality selects directly from the intrinsic shared archive |
| First archive performance pass | `54b437a` | added coverage-checked construction telemetry and removed redundant per-candidate hull work while preserving exact production identities and the current invariance sample |

## Current Contract at `54b437a`

- Compact quality uses three sheetless direct constructors and a bounded
  repeated-family periodic portfolio.
- Only complete canonical-exact endpoints enter the sheetless archive.
- Deduplication and intrinsic ranking occur before requested-sheet q0/q90 fit.
- An invalid, cancelled, deadline-censored, execution-incomplete, or non-fitting
  archive fails; it does not return a partial or ordinary-beam fallback.
- History is one truthful selected terminal snapshot, not invented beam
  ancestry.
- Canonical occupied-union cavities remain separate from requested-sheet
  free-material holes.
- The irregular worker timeout floor is `390,000 ms`.

Current exact one-sheet baselines:

| Fixture | Area | Canonical cavities | Identity evidence |
| --- | ---: | ---: | --- |
| Triangle-20 | `74,428.143126 mm2` | 0 | `371db269...` |
| Mixed-61 | `391,605.850174 mm2` | 0 | fitted production gate `ef2b783a...` |
| Shapes-17 | `304,499.845650 mm2` | 0 | `c640c06f...` |

The `405,773.434053 mm2` pocket-first Mixed endpoint remains an important
direct fallback inside the historical shared-archive research. The
`430,344.917527 mm2` result is the retired fixed-reference baseline. Neither is
the current selected Mixed production result.

## Operational Evidence

The executable and documentary gates are collected in
[`../operations/irregular-production-gates.md`](../operations/irregular-production-gates.md).
Portable accepted and rejected artifacts are indexed from
[`../artifacts/README.md`](../artifacts/README.md).

...
| 3.3 | Cache reuse remains high | **met** | `memory-cache-report.md` §2.2: `pairwise-nfp-relative-v3` hit rate **98.23%** on the real Mixed-61 fixture through the real N-API entry point, within 0.03 points of the TS baseline's 98.20%; `transform-collision-v1` matches TS **exactly** on every counter (10,028/9,540/488/488). |
| 3.4 | Shared-cache contention and duplicate computation are measured and controlled | **met** | `memory-cache-report.md` §2: `staleDetections`/`staleRemovals`/`duplicateComputations`/`singleFlightWaits`/`evictions` all `0` on the real single-threaded production run (expected for this configuration); `cache-concurrency-design.md` §3–4 specifies the sharded-lock + single-flight architecture that prevents duplicate computation under concurrency; misses/stores counters (the numbers that gate actual recomputation cost) match TS exactly. **met-with-note**: telemetry was measured on the single-threaded (compiled-default) configuration, since that is the shipped default (§4 below); a dedicated multi-threaded contention/duplicate-computation telemetry capture (rather than inferring "0 contention possible under 1 thread" logically) was not separately re-run this session — `cache-concurrency-design.md`'s sharded-lock design is the mechanism, not re-measured under load here. |
| 3.5 | Cache insertion order cannot affect behavior | **met** | Cache is documented and empirically confirmed (`differential-e2e-report.md` Finding N1 addendum) as "a pure memoization layer, not a correctness input" — the N1 investigation explicitly tested and disproved a cache-scoping/ordering hypothesis for a real observed divergence, finding the true root cause was an unrelated `hypot` ULP mismatch, not cache insertion order. |
| 3.6 | Cache memory is bounded and cleaned up | **met** | `memory-cache-report.md` §4: job-local `GeometryCacheStore`, one instance per job (`cacheInstances: 1`), Rust ownership drops it at job-frame return; entry counts bounded by one job's own deduplicated key space (4,811 + 488 entries on the full Mixed-61 fixture); peak RSS 0.22–0.35× of TS's on the same workload. |
| 3.7 | No deadlocks, poisoned permanent entries, or leaked native jobs exist | **met** | 710/710 `cargo test --release` green including the full `thread_equality.rs` suite (48 job executions across 4 thread counts with no hang); `cache-concurrency-design.md` §4 single-flight design has no permanent-poison path (panics propagate and release the guard, per that document's own analysis); no job-handle leak mechanism exists structurally — `boundary`'s job lifecycle is owned by Rust's Drop, not a manually-released resource (`native-boundary.md` §6.4). |

...
- The worker-local active-run controller drives `ComputeIrregularNestingOptions.isCancelled` for TypeScript execution and a deferred one-shot native cancellation callback for Rust execution. The callback receives the retained first reason, so cancellation requested before native registration is replayed once with `cancelled` or `timeout`.
...
The native semantic request DTO remains unchanged. Cancellation registration is internal execution state: after encoding the JSON, the TypeScript adapter generates an opaque per-invocation token and passes it separately to native `runIrregularJob`. Native registration is synchronous before its run promise is returned, and cancellation uses that token with the retained `cancelled` or `timeout` reason. Public `jobId` never identifies a native lease, so overlapping native invocations with the same job ID remain independent. Native maps cancellation to `worker_cancelled` with reason `cancelled` and timeout to `worker_timeout` with reason `deadline`. Neither Section 7 nor Section 8 changes accepted outputs, partial-result rules, or typed error classification.

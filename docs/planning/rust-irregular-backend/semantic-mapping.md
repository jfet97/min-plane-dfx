...
| `src/workers/irregular/transformCollisionGeometry.ts` | `caches::transform_collision_geometry` (Effect wrapper collapses into the same resolver at the Rust boundary) | Effect-wrapped caller around the pure `transformCollisionGeometryCore.ts` resolver. | Indirect via `pureIrregularCoreBoundary.test.ts`, `trustedGeometryCarrierBoundary.test.ts` [U] | MEDIUM |
| `src/workers/irregular/core/transformCollisionGeometryCore.ts` | `caches::transform_collision_geometry` (ownership moved to the caches module during Stage 2 wave 3; the nfp_ifp port must import it, never re-implement) | Pure hot-path resolver called directly by `geometryKernel.ts`; transformed-geometry key construction and lookup before recomputation (migration prompt §13.2). Signed-zero fold at `:181`. | `tests/unit/pureIrregularCoreBoundary.test.ts` [U] | HIGH |

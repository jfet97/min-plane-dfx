...
Do not generalize this result into transform reselection, two rows, multiple
folds, a beam, or iterative repair. Reopen construction-time search only for a
useful discarded directional state already generated by an existing producer
at zero extra evaluation cost, followed by one bounded warm continuation.

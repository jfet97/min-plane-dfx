...
The zero-search Stage 1 archive observer remains the first sibling source. Do
not promote the width-one target-aware selector or retain it as an automatic
post-production cost. Its raw completed diagnostic also fails the existing
`0.15` hull-gap floor. Post-run review found incomplete finalization coverage
in budget accounting and disabled-path state retention; `edeed42` removes the
rejected implementation while preserving its history and immutable evidence.
...
The exact one-row shelf then missed Shapes-17 `2000 x 2700` by only
...
The later `d57b7d6` gate corrects the feature-level acceptance contract. A
Compact fallback below `80%` short-edge fill is now a directional miss, never
a passing profile. After a pair miss, one deterministic next-fit shelf reuses
the same transform evaluation and may create multiple AABB-separated rows. It
adds exact roomy Triangle-20 and Mixed-61 winners at `88.288%` and `99.389%`
fill. The full matrix has four generated directional winners, five explicit

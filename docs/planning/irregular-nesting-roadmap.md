...
width-four Stage 4 rejected at `1163bd3`; one-row shelf Stage 5 rejected at
`999e9fb`; production influence remains zero.
...
The later one-row terminal shelf supplied a different exact witness: Shapes-17
reached its final piece in `3.451 ms` and `136` transform evaluations but
required `2007.195 mm` on a `2000 mm` edge. Its implementation was removed
after the failed fit gate. One separately reviewed single-pair AABB fold is now
authorized by that near-boundary evidence. It must enumerate exactly
`n(n-1)/2` pairs, forbid multiple folds and transform reselection, and stop
without escalation if its exact admission or reproduction gates fail.

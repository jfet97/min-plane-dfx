...
When short-side capture is enabled, the same command requires nine genuine
directional outcomes and zero directional misses. Short Side receives the exact
piece partition selected by Compact, but never receives Compact placements.
It must construct a legal directional result for that partition or return the
typed `irregular_no_valid_result` error; returning Compact geometry is not an
allowed outcome.

The protected depth-first and bounded contact-first lanes share one exact
directional comparator. Small partitions may additionally continue through
reverse-depth and canonical piece-ID orders. Every lane uses exact NFP/IFP
legality and canonical-grid contact measurements. The run remains strictly
sequential and single-process. The accepted matrix, exact hashes, and individual
visual review are under
[`../artifacts/compact-short-side-directional-contract/`](../artifacts/compact-short-side-directional-contract/README.md).
Visual review is a first-class promotion gate: do not promote a short-side
change because the numeric matrix passes.

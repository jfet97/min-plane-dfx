...
6. **completed:** the bounded two-placement compact-closure horizon on Triangle
...
Commit `e22f2fe` implemented that observer and the clean run completed in
`139,828.682 ms` with `104,749` evaluations. It produced 24 equal-work
candidate/sibling comparisons from four distinct conditioned candidates and
three distinct sibling states after future-equivalence deduplication. All 63
shadow witnesses completed and no compact-closure comparison passed.

At the first horizon placement, every conditioned candidate was already larger
than its sibling: candidate areas ranged from `95,506.413961` to
`99,443.140496 mm2`, while sibling areas ranged from `87,710.145649` to
`88,104.577995 mm2`. After the second placement, candidates ranged from
`99,028.956145` to `99,443.140496 mm2`; every sibling converged to
`88,124.330 mm2`. The best conditioned endpoint reduced isolates from 11 to
nine and could raise contacts from five to six, but retained 14 contact
components with a largest component of three. None reached the exact common-
